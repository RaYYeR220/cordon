/**
 * Wallet discovery, connection and teardown.
 *
 * No React and no DOM rendering: this module only talks to the wallet-standard registry and to
 * Starknet.js, so it is usable from a plain script, a test, or a framework this package has never
 * heard of. `useCordonWallet` is a thin layer over it.
 */
 ...
 *
 * `eip1193Adapters: []` is load-bearing. The default adapter list bridges EIP-6963 providers into
 * the registry, which makes MetaMask's Starknet Snap get probed on every discovery pass and spams
 * its unlock popup. Passing an empty list keeps discovery to wallets that registered as Starknet
 * wallets. It is also why this package does not use starknetkit's `connect()`: that bundles
...
/**
 * Wallets worth offering. MetaMask is filtered out: its Starknet support goes through a Snap that
 * has no STRK20 methods and interrupts the flow with an unlock prompt.
 */
...
async function readOptional<T>(read: () => Promise<T>, fallback: T): Promise<T> {
  try {
    ...
    // Optional metadata. A wallet that does not answer is not a failure to connect, and the
    // capability probe reports what is missing.
    return fallback;
  }
}
...
/**
 * Connect to a discovered wallet and build the `WalletAccountV6` the STRK20 methods hang off.
 *
 * The RPC provider is the caller's, not the wallet's: a wallet's own provider is fixed at connect
 * time and can point at a different network than the one the app reads from.
 */
...
/**
 * Ask the wallet to drop the session through `standard:disconnect`.
 *
 * Wallets are free to ignore it, so the caller must clear its own state regardless — the boolean
 * says whether the wallet acknowledged, not whether the UI should reset.
 */
