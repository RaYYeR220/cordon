 ...
 * Error normalisation.
 *
 * The rule for the whole package: show what the wallet or the node actually said. Nothing here
 * invents a message, and nothing swallows one — an unrecognised error keeps its original text and
 * reports `source: "unknown"`.
 *
 * Decoding a Cordon panic code into the rule that fired is `@cordon/sdk`'s job, not this module's.
 * This one only lifts the raw code out of whatever shape the transport threw.
 */

import { shortString } from "starknet";
import type { Strk20NormalizedError } from "./types.js";
...
function stringifyData(data: unknown): string {
  ...
  if (isRecord(data)) {
    const revert = data["revert_error"];
    const execution = data["execution_error"];
    if (typeof revert === "string") return revert;
...
/**
 * Decode a felt that encodes a Cairo short string. Returns null when the felt is not printable
 * ASCII, so numeric failure codes are left alone rather than rendered as mojibake.
 */
...
/**
 * Lift the Cairo revert reason out of a node error and decode any short-string panic codes inside
 * it. Returns the raw reason text plus the decoded codes.
 */
export function extractRevert(text: string): { reason: string | null; panicCodes: string[] } {
  ...
  const reason =
    match?.[1]?.trim() ?? (/revert|Execution failed|panic/i.test(text) ? text.trim() : null);
  if (!reason) return { reason: null, panicCodes: [] };

  const codes = new Set<string>();

  // Nodes that decoded the short string themselves print it as ('CORDON_OVER_CAP').
  for (const quoted of reason.matchAll(QUOTED_RE)) {
    if (quoted[1]) codes.add(quoted[1]);
  }
  // Nodes that did not print raw felts; decode them here.
...
export function normalizeError(error: unknown): Strk20NormalizedError {
  ...
  if (code !== null && WALLET_ERROR_NAMES[code]) {
    source = "wallet";
    name = WALLET_ERROR_NAMES[code] ?? null;
  } else if (code !== null && RPC_ERROR_NAMES[code]) {
    source = "rpc";
    name = RPC_ERROR_NAMES[code] ?? null;
  } else if (code !== null) {
    // A code we have no name for is still a real code — report it as-is.
...
/**
 * True when a wallet is telling us the STRK20 method does not exist.
 *
 * Wallets are inconsistent here: Braavos answers with a plain "Not implemented" message rather
 * than a spec error code, so this matches on the message as well as on the JSON-RPC "method not
 * found" code.
 */
...
/** A single line safe to render, carrying the code and the revert reason. */
...
/** Build a normalised error from a plain message, for failures raised in this package. */
export function localError(message: string): Strk20NormalizedError {
