...
import { WAIT_RETRIES, WAIT_RETRY_INTERVAL_MS } from "./config";
import { normalizeError } from "./errors";
import type { Strk20Action, Strk20NormalizedError, Strk20SubmitResult } from "./types";
...
/**
 * Pull the parts of a receipt we report on. The receipt is a union of several
 * spec shapes across RPC versions, so it is read defensively: a field we cannot
 * find is reported as null rather than defaulted.
 */
 ...
 *
 * The wallet generates a STARK proof before it can even submit, and the proof is
 * verified on-chain afterwards, so both halves are slow — the wait budget is
 * deliberately generous (400 x 3s by default) and a timeout is reported as
 * `unconfirmed`, never as a failure: the transaction may well land later.
 */
  ...
  try {
    ...
    return {
      ...
      result: {
        ...
        error: reverted
          ? normalizeError(
              new Error(parsed.revertReason ?? "Transaction reverted without a reason in the receipt.")
            )
          : null,
