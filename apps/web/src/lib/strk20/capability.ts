 ...
 *
 * The STRK20 methods are optional: a wallet can speak the wallet API in full and
 * still not implement them. Today only Ready does — Braavos answers
 * `wallet_strk20Balances` with "Not implemented". So the app never assumes; it
 * asks, using the one STRK20 method that is read-only and free.
 *
 * `wallet_strk20Balances` is the right probe because it signs nothing, submits
 * nothing and costs nothing, and its success payload is the shielded balance
 * list the app needs anyway.
 */

import { chainName } from "./config";
import { isNotImplemented, isUserRefusal, normalizeError } from "./errors";
import { parseBalances } from "./balances";
import type { ConnectedWallet } from "./wallet";
import type { Strk20Capability } from "./types";
  ...
  try {
    ...
    if (isNotImplemented(error)) {
      return {
        ...
        reason:
          ...
          "Private actions need a wallet that does — Ready is the only one shipping them today.",
        probe,
        balances: null,
        error,
      };
    ...
    if (isUserRefusal(error)) {
      return {
        ...
        status: "declined",
        reason:
          `${connection.name} was asked but the request was declined, so STRK20 support is unknown.`,
        probe,
        balances: null,
        error,
      };
...
/** Whether private actions can be attempted at all. */
