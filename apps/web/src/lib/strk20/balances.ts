/**
 * Balance reads. Shielded balances come from the wallet (only it holds the
 * viewing key); public balances come from a plain ERC-20 call over RPC.
 */

import { num, uint256, type ProviderInterface } from "starknet";

import { normalizeError } from "./errors";
import type { Address, Strk20Balance, Strk20NormalizedError } from "./types";

/** A value we could not read. The UI renders this, it never renders a guess. */
 ...
 *
 * The spec field is `balance`, but wallets have shipped `amount` too, so both
 * are accepted. An entry we cannot read is dropped rather than reported as zero.
 */
...
export async function readPublicBalance(
  ...
  try {
    ...
    const value =
      result[1] === undefined
        ? num.toBigInt(low)
        : uint256.uint256ToBN({ low, high: result[1] });
